...
const Register = () => {
  ...
  return (
    <div className="min-h-[85vh] flex items-center justify-center px-6 bg-[radial-gradient(ellipse_at_top,var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-slate-950 py-12">
      <div className="w-full max-w-md rounded-2xl bg-slate-900/40 border border-slate-800/80 p-8 backdrop-blur-xl shadow-2xl">
        <div className="text-center space-y-2 mb-8">
          ...
          <p className="text-sm text-slate-400">Join SportNest to unlock global arenas</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-1.5">
            <label className="text-xs font-bold uppercase tracking-wider text-slate-400">Full Name</label>
            <input type="text" name="name" value={formData.name} onChange={handleChange} className="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 focus:border-emerald-500 focus:outline-none text-sm transition-colors text-slate-100" placeholder="John Doe" />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input type="url" name="photoUrl" value={formData.photoUrl} onChange={handleChange} className="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 focus:border-emerald-500 focus:outline-none text-sm transition-colors text-slate-100" placeholder="https://example.com/photo.jpg" />
          </div>
